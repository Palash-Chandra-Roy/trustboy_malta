...
class LoginBloc extends Bloc<LoginEvent, LoginStateModel> {
  ...
  LoginBloc({required AuthRepository repository})
    ...
    on<SaveUserCredentialEvent>((event, emit) {
      emit(state.copyWith(isActive: event.isActive, loginState: const LoginStateInitial()));
    });
    ...
    on<LoginEventLanguageCode>((event, emit) {
      emit(state.copyWith(langCode: event.languageCode));
    });
    on<LoginEventRememberMe>((event, emit) {
      emit(state.copyWith(isActive: !state.isActive));
    });
    on<LoginEventNavTab>((event, emit) {
      //debugPrint('navTab ${event.isLastTab}');
      emit(state.copyWith(isLastTab: event.isLastTab));
    });
  ...
  /*Future<void> _logoutEvent(LoginEventLogout event, Emitter<LoginStateModel> emit) async {
    final uri = Utils.tokenWithCode(RemoteUrls.logout, userInformation?.accessToken?? '', state.langCode);
    emit(state.copyWith(loginState: LoginStateLogoutLoading()));
    final result = await _repository.logout(uri);
    result.fold(
      (failure) {
  ...
  Future<void> _logoutEvent(LoginEventLogout event, Emitter<LoginStateModel> emit) async {
    ...
    final result = await _repository.clearLocalUser();
    result.fold(
          (failure) {
        final error = LoginStateLogoutError(failure.message, failure.statusCode);
        emit(state.copyWith(loginState: error));
      },
          (success) {
        _users = null;
        const loaded  = LoginStateLogoutLoaded('Successfully Logout',200);
