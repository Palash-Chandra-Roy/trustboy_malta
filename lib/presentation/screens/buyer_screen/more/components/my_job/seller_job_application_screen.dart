...
import '../../../../job/component/seller_job_application_card.dart';
import '../../../../user_screen/order/components/empty_order.dart';

class SellerJobApplicationScreen extends StatefulWidget {
  const SellerJobApplicationScreen({super.key});

  @override
  State<SellerJobApplicationScreen> createState() => _SellerJobApplicationScreenState();
}

class _SellerJobApplicationScreenState extends State<SellerJobApplicationScreen> {
  ...
  _initState() {
    ...
    Future.microtask(()=>serviceCubit.getJobReqList());
...
class _LoadedJobPostViewState extends State<LoadedJobPostView> {
  ...
  late List<List<JobReqItem>?> filteredList;
  ...
  _init(){
    ...
    filteredList = [
      widget.jobPostList?.sellerJobsReq??[],
      widget.jobPostList?.sellerHiredJobsReq??[],
      widget.jobPostList?.sellerPendingJobsReq??[],
      widget.jobPostList?.sellerRejectedJobsReq??[],
    ];
  }
      ...
      child: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            centerTitle: true,
            title: CustomText(
              text: Utils.translatedText(context, "My Job Applications"),
              fontSize: 18.0,
            ...
            SliverList.list(
              children: List.generate(
                filteredList[jCubit.state.totalView]?.length??0,
                  ...
                  return  SellerJobApplicationCard(item: item);
                },
              ),
            ),
          ] else ...[
            const EmptyOrder(empty: 'Job Application Empty',isShowSub: false),
          ],
        ],
      ),
    );
      ...
      child: BlocBuilder<JobPostCubit, JobPostItem>(
        builder: (context,state){
          List<String> orderTabItems = [
            Utils.translatedText(context, 'All(${jCubit.jobPost?.sellerJobsReq?.length??0})'),
            Utils.translatedText(context, 'Hired(${jCubit.jobPost?.sellerHiredJobsReq?.length??0})'),
            Utils.translatedText(context, 'Pending(${jCubit.jobPost?.sellerPendingJobsReq?.length??0})'),
            Utils.translatedText(context, 'Rejected(${jCubit.jobPost?.sellerRejectedJobsReq?.length??0})'),
          ];
          return Padding(
