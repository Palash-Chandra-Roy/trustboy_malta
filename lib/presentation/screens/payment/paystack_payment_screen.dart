...
class PaystackPaymentScreen extends StatefulWidget {
  const PaystackPaymentScreen({super.key, required this.url});
  final Uri url;

  @override
  State<PaystackPaymentScreen> createState() => _PaystackPaymenttState();
}

class _PaystackPaymenttState extends State<PaystackPaymentScreen> {
  double value = 0.0;
  ...
  void initializeController() {
    ...
    controllerGlobal = WebViewController()
      ...
      ..loadRequest(widget.url, method: LoadRequestMethod.get, headers: header);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: DefaultAppBar(title: Utils.translatedText(context, 'Paystack Payment')),
      body: Column(
        children: [
          if (_isLoading)
            Center(
  ...
  void _redirect(String url) {
    if (_canRedirect) {
      bool isSuccess = url.contains('/webview-success-payment') && url.contains(RemoteUrls.rootUrl);
  ...
  /*void getData() {
    controllerGlobal.runJavaScriptReturningResult("document.body.innerText").then(
      (data) {
        try{
          var decodedJSON = jsonDecode(data.toString());
          var responseJSON = jsonDecode(decodedJSON);
          log(decodedJSON, name: 'PaystackPaymentScreen');
          ...
          debugPrint('enter-catch-block');
          throw Exception('Exception occurred ${e.toString()} - $t');
        }finally {
...
///working code
  /*void getData() {
    controllerGlobal.runJavaScriptReturningResult("document.body.innerText").then(
          (data) {
        try{
          log("Raw data from WebView: $data");
          ...
          final Map<String, dynamic> responseJSON = jsonDecode(rawJson);

          log("Decoded JSON: $responseJSON", name: 'PaystackPaymentScreen');
