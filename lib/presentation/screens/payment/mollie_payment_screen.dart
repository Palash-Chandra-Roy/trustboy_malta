...
class MolliePaymentScreen extends StatefulWidget {
  const MolliePaymentScreen({super.key, required this.url});
  final Uri url;

  @override
  State<MolliePaymentScreen> createState() => _MolliePaymentState();
}

class _MolliePaymentState extends State<MolliePaymentScreen> {
  double value = 0.0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: DefaultAppBar(title: Utils.translatedText(context, 'Mollie Payment')),
      body: Column(
        children: [
          if (_isLoading)
            Center(
