import 'package:flutter/material.dart';

import 'utils.dart';

const secondaryColor = Color(0xFF13544E);
const primaryColor = Color(0xFF22BE0D);
const blackColor = Color(0xFF06131C);
// const blackColor = Color(0xFF061533);

const grayColor = Color(0xFF535769);
const gray5B = Color(0xFF5B5B5B);
Color inputTextColor = const Color(0xFFBABABA);
const greenColor = Color(0xFF22C55E);
const blueColor = Color(0xFF4B83FC);
const redColor = Color(0xFFFE2C55);
const whiteColor = Color(0xFFFFFFFF);
const stockColor = Color(0xFFEDEBE7);

const scaffoldBgColor = Color(0xFFF7F5F0);
const grayBackgroundColor = Color(0xFFF3F3F3);
const Color borderColor = Color(0xFFE2E8F0);
const Color cardTopColor = Color(0xFFD4E9C8);

const kDuration = Duration(milliseconds: 800);

const Color transparent = Colors.transparent;
const double dialogHeight = 270.0;

///custom fonts
const String bold400 = 'Regular400';
const String bold500 = 'Regular500';
const String bold700 = 'Bold700';

  OutlineInputBorder outlineBorder([double radius = 50.0]) => OutlineInputBorder(
   borderSide:  const BorderSide(color: stockColor),
   borderRadius: Utils.borderRadius(r:radius),
 );


///gradient colors
const buttonGradient = LinearGradient(
  begin: Alignment(0.00, -1.00),
  end: Alignment(0, 1),
  colors: [Color(0xFF885DF1), Color(0xFF6610F2)],
);

const activeTabButtonGradient = LinearGradient(
  begin: Alignment(0.00, -1.00),
  end: Alignment(0, 1),
  colors: [Color(0xFFFFC107), Color(0xFFFD7E14)],
);
const inactiveTabButtonGradient = LinearGradient(
  begin: Alignment(0.00, -1.00),
  end: Alignment(0, 1),
  colors: [Colors.white, whiteColor],
);

const dialogCircleGradient = LinearGradient(
  begin: Alignment(0.00, -1.00),
  end: Alignment(0, 1),
  colors: [Color(0xFFFFC107), Color(0xFFFD7E14)],
);
